...
Companion (/extract endpoint)
  |
  +-- Cache hit: ~/.grapply/parsers/{domain}.py exists
...
MCP Server (subprocess, stdio transport)
  Tools:
    ...
    save_parser(domain, code)   - write ~/.grapply/parsers/{domain}.py
...
```python
# ~/.grapply/parsers/linkedin.com.py
...
server = Server('grapply-parser-tools')
...
async def call_tool(name: str, arguments: dict) -> list[types.TextContent]:
    ...
    if name == 'save_parser':
        ...
        header = (
            f'# Generated by grapply MCP parser agent\n'
            f'# Domain: {domain}\n'
...
def _run_cached_parser(path: Path, text: str) -> dict | None:
    ...
    try:
        ...
        print(f'[grapply] Parser {path.name} failed: {e}')
    return None
...
def extract(domain: str, page_text: str, ai_client) -> dict:
    ...
    if parser_path.exists():
        ...
        if result:
            return result
        print(f'[grapply] Parser for {domain} failed, regenerating via MCP...')
        parser_path.unlink()

    # Slow path: MCP agentic loop
    print(f'[grapply] No parser for {domain} - starting MCP agent...')
...
Parsers live in `~/.grapply/parsers/` - gitignored, user-local.
