...
| 4 | `tracker.py` SQLite DB at `~/.grapply/applications.db` - no schema migration logic. If the schema changes in a future version, existing installs will break silently or crash. | Low | **Fixed** — `PRAGMA user_version` tracked as `_SCHEMA_VERSION = 1`. `_migrate()` runs on every `_db()` open, applies pending `_MIGRATIONS` dict entries in order. Adding a column: bump `_SCHEMA_VERSION`, add SQL to `_MIGRATIONS[old_version]`. |
